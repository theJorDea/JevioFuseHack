<div align="center">
  ...
  <p>
    ...
    <a href="#тестирование"><img alt="150 тестов" src="https://img.shields.io/badge/tests-150-22A06B"></a>
...
`supersedes`. Ответ `remember` привязывается к append-only журналу отдельным
...
До granular deletion на реальном Cognee Cloud tenant 11 июля 2026 года был
получен baseline, который зафиксировал проблему, а не скрывал её:
...
Теперь тот же сценарий проверяет физическое удаление и выводит число удалённых
remote sources. Новый Cloud-результат следует публиковать только после повторного
`npm run benchmark:memory`; старый baseline сохранён как честная точка сравнения.
