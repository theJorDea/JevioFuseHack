...
Ollama/API  symbol index + workspace + permission gate
       \\        Cognee semantic memory (optional)
...
src/tools.ts содержит schema и выполнение built-in tools. src/mcp.ts реализует
MCP stdio lifecycle, pagination tools/list и tools/call, а затем добавляет
namespaced schemas в тот же agent tool registry. Проверка workspace и
permission gate находятся на стороне host, а не модели; MCP annotations
считаются недоверенными подсказками.
...
9. Полученная из Cognee память является данными, а не пользовательской инструкцией.
10. Недоступность внешней памяти не должна прерывать задачу или запись Markdown-сессии.
11. MCP-серверы отключены по умолчанию, а их вызовы требуют отдельного approval.
...
- MCP Streamable HTTP, resources и prompts;
