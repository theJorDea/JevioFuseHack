...
Офлайн-набор после синхронизации с `origin/main`: **83/83 теста проходят**,
...
Окончательное подтверждение требует запуска на реальном tenant Cognee с
`COGNEE_BASE_URL` и `COGNEE_API_KEY`.
...
Следующий шаг — добавить `supersedes`, удаление или понижение веса устаревших
записей и provenance отдельных фрагментов ответа Cognee. Риск persistent memory
poisoning отдельно выделен в
...
| --- | --- | --- |
| Реализовано | Настоящий Cognee Cloud lifecycle | permanent и session lifecycle покрыты; нужен запуск с секретами tenant |
| Готово | Hybrid session + graph memory | session и graph recall объединяются; improve переносит активную сессию |
| Частично | Структурированные записи с commit/verification/provenance | `/memory explain` готов; для stale записей ещё нужен `supersedes` |
