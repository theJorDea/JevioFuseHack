...
Офлайн-набор содержит **158 unit-тестов** плюс отдельный Cloud integration test,
...
tenant Cognee за 31 секунду, включая granular source lookup/delete и cleanup
временного dataset.
...
### P0 — защита от устаревшей и отравленной памяти: реализовано с legacy fallback
...
```json
{
  ...
  "remote": {
    "dataId": "cognee-data-id",
    "datasetId": "cognee-dataset-id",
    "pipelineRunId": "cognee-run-id",
    "contentHash": "local-sha256",
    "remoteContentHash": "cognee-content-hash"
  }
}
```

`/memory replace <record-id> <текст>` создаёт новую append-only provenance-запись,
заменяет старый текст в `MEMORY.md`, физически удаляет адресуемый Cognee source
по `dataId` и отправляет replacement. Remote receipt также добавляется в журнал
append-only событием. CLI и Web host до model prompt удаляют tombstone-строки и
фрагменты с ID superseded-записей; `/memory explain` показывает обе стороны связи
и remote IDs/hash. Если Cloud `remember` не возвращает `dataId`, Jevio находит
data item через dataset listing и подтверждает его raw-content hash. Для legacy-записей без receipt остаётся ID-фильтр; абсолютная
гарантия для уже созданных старых summaries требует разового reindex dataset.
...
### P1 — наблюдаемость и повторные попытки: базовый слой реализован

Jevio создаёт OpenTelemetry spans для task, model, tools и Cognee HTTP, добавляет
verification events и token usage, если provider её возвращает. Доступны console
и OTLP exporters, sampling и graceful shutdown. Для безопасных Cognee операций
реализованы `Retry-After`, exponential backoff и jitter на `429/502/503/504`;
permanent `remember` намеренно не повторяется. Дальше можно добавить:
...
- агрегированные latency/error metrics;
- recall hit-rate и pipeline run ID как отдельные events;
- tail sampling в Collector для production.
...
### 4. OpenTelemetry: реализовано

Реализованы опциональные console/OTLP exporters и spans:
...
| Реализовано | Retrieval benchmark Cognee on/off | granular deletion 4/4; Cognee on — 20/20, recall 100%, stale errors 0% против прежних 16/20 и 100% stale errors |
| Готово | Hybrid session + graph memory | session и graph recall объединяются; improve переносит активную сессию |
| Готово | Структурированные записи с commit/verification/provenance | `/memory explain`, remote receipt, granular delete, `supersedes` и host-side fallback готовы |
...
| Готово | OpenTelemetry | один task отображается единым trace без секретов и file contents |
| Готово | Retrieval memory benchmark | отчёт Cognee off/on воспроизводится одной командой и защищён CI quality gate |
| Готово | Coding benchmark harness | memory-dependent fixtures запускаются через реальную модель в режимах off/on |
