...
function parseAskOptions(raw: unknown, max = 8): AskUserOption[] {
 if (!Array.isArray(raw)) return [];
 return raw.slice(0, max).flatMap((item): AskUserOption[] => {
 if (!item || typeof item !== "object") return [];
 const value = item as Record<string, unknown>;
 const label = String(value.label ?? "").trim();
 return label
 ? [{ label, description: typeof value.description === "string" ? value.description : undefined }]
 : [];
 });
}

/** Normalize tool input into AskUserRequest (legacy single question + multi form). */
export function parseAskUserRequest(input: Record<string, unknown>): AskUserRequest {
 const header = typeof input.header === "string" ? input.header.trim() || undefined : undefined;
 const fromBatch = Array.isArray(input.questions)
 ? input.questions.slice(0, 4).flatMap((item, index): AskUserQuestion[] => {
 if (!item || typeof item !== "object") return [];
 const value = item as Record<string, unknown>;
 ...
 question,
 options: parseAskOptions(value.options),
 multiSelect: value.multi_select === true,
 allowOther: value.allow_other !== false,
 }];
 })
 : [];

 if (fromBatch.length) return { header, questions: fromBatch };

 const question = String(input.question ?? "").trim();
 if (!question) return { header, questions: [] };
 return {
 header,
 questions: [{
 id: "q1",
 question,
 options: parseAskOptions(input.options),
...
const definitions: Record<string, ToolDefinition> = {
 ...
 function: {
 name: "git_diff",
 description: "Show git status plus staged and unstaged diffs (no repo changes). Includes untracked paths via status.",
 parameters: { type: "object", properties: {}, additionalProperties: false },
 },
 },
 load_skill: {
 ...
 name: "ask_user",
 description:
 "REQUIRED for open product/design choices: pause and show the user a multiple-choice form. Call this BEFORE write_file when style, layout, stack, features, or content are underspecified — never invent preferences or only list options in Markdown. Prefer 2–5 short options; use questions[] for 1–4 form fields; multi_select when several can apply. Do not ask for facts readable from the workspace. One focused form per decision point.",
 parameters: {
 type: "object",
 properties: {
 header: {
 type: "string",
 description: "Optional short context line shown above the form (why you need this).",
 },
 question: {
 type: "string",
 description: "Single decision question (legacy). Prefer questions[] for multiple.",
 },
 options: {
 type: "array",
 maxItems: 8,
 description: "Options for a single question (legacy).",
 items: {
 type: "object",
 properties: {
 label: { type: "string" },
 ...
 multi_select: {
 type: "boolean",
 description: "For a single question: allow multiple option picks (default false).",
 },
 questions: {
 type: "array",
 maxItems: 4,
 description: "Batch of 1–4 clarifying questions shown as a form.",
 items: {
 type: "object",
 properties: {
 id: { type: "string", description: "Short id for the answer map (optional)" },
 question: { type: "string" },
 multi_select: { type: "boolean" },
 allow_other: { type: "boolean", description: "Show free-text Other (default true)" },
 options: {
 type: "array",
 maxItems: 8,
 items: {
 ...
 name: "update_todo",
 description:
 "Maintain the live ToDo checklist shown in the TUI. REQUIRED for multi-step implementation: create the list before edits, keep exactly one item in_progress, mark completed only after tool evidence, send the FULL list each call (replaces previous). Max 12 concrete items. Do not use for pure one-line Q&A.",
 parameters: {
 type: "object",
 properties: {
 todos: {
 ...
 description: "pending | in_progress (one at a time) | completed",
 },
 },
...
export async function executeTool(name: string, input: Record<string, unknown>, context: ToolContext): Promise<string> {
 ...
 const request = parseAskUserRequest(input);
 if (!request.questions.length) throw new Error("Provide question or questions[] with at least one item.");
 return context.askUser(request);
 }
 ...
 // Soft-normalize: keep first in_progress, demote the rest to pending.
 let seen = false;
 ...
 const note = inProgress.length > 1
 ? "\n(Note: only one in_progress allowed — extra items demoted to pending.)"
 : "";
 ...
 if (name === "git_diff") {
 try {
 const execGit = (args: string[]) => execFileAsync("git", args, {
 cwd: context.workspace,
 maxBuffer: MAX_OUTPUT * 2,
 windowsHide: true,
 });
 ...
 execGit(["diff", "--no-ext-diff", "--cached", "--"]),
 execGit(["diff", "--no-ext-diff", "--"]),
 ]);
 const sections: string[] = [];
 if (status.stdout.trim()) sections.push(`## Status\n${status.stdout.trim()}`);
 if (staged.stdout.trim()) sections.push(`## Staged\n${staged.stdout.trim()}`);
 if (unstaged.stdout.trim()) sections.push(`## Unstaged\n${unstaged.stdout.trim()}`);
 // Surface untracked paths from status for reviewers when there is no text diff yet.
 if (!sections.length) return "Working tree is clean (no staged/unstaged changes; status empty).";
